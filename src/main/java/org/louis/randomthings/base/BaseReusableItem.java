...
public class BaseReusableItem extends BaseItem {
    ...
    public BaseReusableItem(int uses, boolean tooltip, Function<Item.Properties, Item.Properties> properties) {
        super(p -> properties.apply(new Item.Properties()));  // Truyền Function<Item.Properties, Item.Properties> vào
        this.damage = uses > 0;
    ...
    public void appendHoverText(ItemStack pStack, @Nullable Level pLevel, List<Component> tooltip, TooltipFlag pIsAdvanced) {
        if (this.tooltip) {
            if (this.damage) {
                ...
                if (damage == 1) {
                    tooltip.add(Tooltips.ONE_USE_LEFT.getTextComponent());
                } else {
                    tooltip.add(Tooltips.USES_LEFT.args(damage).getTextComponent());
                }
            } else {
                tooltip.add(Tooltips.UNLIMITED_USES.getTextComponent());
            }
        }
    }
}
