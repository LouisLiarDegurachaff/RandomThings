package org.louis.randomthings.util.generator;
...
public class FurnaceFuelSlot extends SlotItemHandler {
    ...
    public FurnaceFuelSlot(IItemHandler inventory, int index, int xPosition, int yPosition) {
        this(inventory, index, xPosition, yPosition, stack -> ForgeHooks.getBurnTime(stack, RecipeType.SMELTING) > 0);
    }

    public FurnaceFuelSlot(IItemHandler inventory, int index, int xPosition, int yPosition, Predicate<ItemStack> fuelPredicate) {
